 ...
 ***************************************************************************/
	...
	$result = mysql_query ( "SELECT * FROM ".$DBPrefix."auctions WHERE closed='0' AND suspended = '0' AND current_bid>0 ORDER BY current_bid DESC LIMIT ".intval($offset).",".intval($limit));
		...
		while ($row=mysql_fetch_array($result))
		{
			...
			$tplv .= "<TR ALIGN=CENTER VALIGN=MIDDLE BGCOLOR=\"$bgColor\">";
				...
				/* time left till the end of this subastas */
					...
					$tplv .= "<TD>$days_difference $MSG_097 <BR>$hours_difference:$minutes_difference:$seconds_difference</TD>";
	...
	include "header.php";
	include phpa_include("template_view_higher_php.html");
	include "footer.php";
