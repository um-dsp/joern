<? /* these functions are used to interact with the DB */
	...
	function processMessages()
	{
		...
		if ($_POST['requestDraw'] == "yes")
		{
			...
			if ($_SESSION['isSharedPC'])
			{
				$tmpQuery = "UPDATE games SET gameMessage = 'draw', messageFrom = '".$currentPlayer."' WHERE gameID = ".$_SESSION['gameID'];
				mysql_query($tmpQuery);
			}
			else
			{
		...
		if (isset($_POST['undoResponse']))
		{
			if ($_POST['isUndoResponseDone'] == 'yes')
			{
				...
				$tmpQuery = "UPDATE messages SET msgStatus = '".$tmpStatus."', destination = '".$opponentColor."' WHERE gameID = ".$_SESSION['gameID']." AND msgType = 'undo' AND msgStatus = 'request' AND destination = '".$currentPlayer."'";
				mysql_query($tmpQuery);
		...
		if (isset($_POST['drawResponse']))
		{
			if ($_POST['isDrawResponseDone'] == 'yes')
			{
				...
				$tmpQuery = "UPDATE messages SET msgStatus = '".$tmpStatus."', destination = '".$opponentColor."' WHERE gameID = ".$_SESSION['gameID']." AND msgType = 'draw' AND msgStatus = 'request' AND destination = '".$currentPlayer."'";
				mysql_query($tmpQuery);
		...
		if ($_POST['resign'] == "yes")
		{
			$tmpQuery = "UPDATE games SET gameMessage = 'playerResigned', messageFrom = '".$currentPlayer."' WHERE gameID = ".$_SESSION['gameID'];
			mysql_query($tmpQuery);
		...
		/* ******************************************* */
		$tmpQuery = "SELECT * FROM messages WHERE gameID = ".$_SESSION['gameID']." AND destination = '".$currentPlayer."'";
				...
				case 'undo':
					switch($tmpMessage['msgStatus'])
					{
						case 'request':
							...
							break;
						case 'approved':
							$tmpQuery = "DELETE FROM messages WHERE gameID = ".$_SESSION['gameID']." AND msgType = 'undo' AND msgStatus = 'approved' AND destination = '".$currentPlayer."'";
							mysql_query($tmpQuery);
							$statusMessage .= "Undo approved.<br>\n";
							break;
						case 'denied':
							$isUndoing = false;
							$tmpQuery = "DELETE FROM messages WHERE gameID = ".$_SESSION['gameID']." AND msgType = 'undo' AND msgStatus = 'denied' AND destination = '".$currentPlayer."'";
							mysql_query($tmpQuery);
				...
				case 'draw':
					switch($tmpMessage['msgStatus'])
					{
						case 'request':
							...
							break;
						case 'approved':
							$tmpQuery = "DELETE FROM messages WHERE gameID = ".$_SESSION['gameID']." AND msgType = 'draw' AND msgStatus = 'approved' AND destination = '".$currentPlayer."'";
							mysql_query($tmpQuery);
							$statusMessage .= "Draw approved.<br>\n";
							break;
						case 'denied':
							$tmpQuery = "DELETE FROM messages WHERE gameID = ".$_SESSION['gameID']." AND msgType = 'draw' AND msgStatus = 'denied' AND destination = '".$currentPlayer."'";
							mysql_query($tmpQuery);
		...
		if ($_POST['isCheckMate'] == 'true')
			mysql_query("UPDATE games SET gameMessage = 'checkMate', messageFrom = '".$currentPlayer."' WHERE gameID = ".$_SESSION['gameID']);
