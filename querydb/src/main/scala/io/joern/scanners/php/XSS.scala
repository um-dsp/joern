...
object XSS extends QueryBundle {
  ...
  def XSS()(implicit context: EngineContext): Query =
    Query.make(
      ...
      implicit val attack_san_functions: List[String] = Constants.san_functions_xss

      def source = cpg.call.name(Operators.assignment).argument.code(Constants.attacker_input) 

      def sink = cpg.call.name("print|echo|printf").argument.filterNot(SanitizationFilter.isSanitized(_))
